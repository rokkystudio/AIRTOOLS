...
try{
  Write-Host "UART_TFTP_PUTBACK_OPEN COM3 LOG=$LogPath"
  Read-For 300
  $sp.Write("`r")
  Read-For 500
  Send-Line 'cd /tmp; ls -l mtd4_connectivity.bin' 2000
  Send-Line 'cd /tmp; tftp -p -l mtd4_connectivity.bin -r mtd4_connectivity.roundtrip.bin 192.168.10.36' 90000
  Send-Line 'echo PUTBACK_DONE' 2000
} finally {
  if($sp.IsOpen){$sp.Close()}
  [IO.File]::WriteAllText($LogPath, $buf.ToString(), [Text.UTF8Encoding]::new($false))
  Write-Host "`nUART_TFTP_PUTBACK_CLOSED bytes=$($buf.Length) log=$LogPath"
}
