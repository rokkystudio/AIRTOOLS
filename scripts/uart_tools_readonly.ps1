...
function Send-Line([string]$cmd,[int]$wait=1400){
...
try{
  Write-Host "UART_TOOLS_OPEN COM3 LOG=$LogPath"
  Read-For 500
  $sp.Write("`r")
  Read-For 500
  Send-Line 'echo READY' 1000
  Send-Line 'ls /bin' 3000
  Send-Line 'ls /sbin' 3000
  Send-Line 'ls /usr/bin 2>/dev/null' 3000
  Send-Line 'ls /usr/sbin 2>/dev/null' 3000
  Send-Line 'busybox 2>&1' 5000
  Send-Line 'cat /proc/filesystems' 2000
  Send-Line 'cat /proc/meminfo | head -n 5 2>/dev/null || cat /proc/meminfo' 3000
  Send-Line 'ls /tmp; ls -ld /tmp /var /media' 2500
  Send-Line 'cat /proc/net/dev' 2000
} finally {
  if($sp.IsOpen){$sp.Close()}
  [IO.File]::WriteAllText($LogPath, $buf.ToString(), [Text.UTF8Encoding]::new($false))
  Write-Host "`nUART_TOOLS_CLOSED bytes=$($buf.Length) log=$LogPath"
}
