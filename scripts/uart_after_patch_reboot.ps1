...
try{
  Write-Host "UART_REBOOT_OPEN COM3 LOG=$LogPath"
  Start-Sleep -Milliseconds 300
  try { $s=$sp.ReadExisting(); if($s.Length -gt 0){ [Console]::Write($s); [void]$buf.Append($s) } } catch {}
  Write-Host "SEND_REBOOT"
  $sp.Write("reboot`r")
  $sw=[Diagnostics.Stopwatch]::StartNew()
  while($sw.ElapsedMilliseconds -lt 100000){
    try { $s=$sp.ReadExisting(); if($s.Length -gt 0){ [Console]::Write($s); [void]$buf.Append($s) } } catch {}
  ...
  Write-Host "`nUART_REBOOT_CLOSED bytes=$($buf.Length) log=$LogPath"
}
