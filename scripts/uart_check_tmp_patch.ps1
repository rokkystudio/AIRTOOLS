...
function Send-Line([string]$cmd,[int]$wait=1200){
...
try{
  Write-Host "UART_CHECK_OPEN COM3 LOG=$LogPath"
  Read-For 500
  $sp.Write("`r")
  Read-For 500
  Send-Line 'cd /tmp; ls -l mtd4_connectivity.bin; wc -c mtd4_connectivity.bin 2>/dev/null || echo NO_WC' 5000
  Send-Line 'cd /tmp; test -s mtd4_connectivity.bin && echo FILE_EXISTS || echo FILE_MISSING' 2000
  Send-Line 'free' 2000
} finally {
  if($sp.IsOpen){$sp.Close()}
  [IO.File]::WriteAllText($LogPath, $buf.ToString(), [Text.UTF8Encoding]::new($false))
  Write-Host "`nUART_CHECK_CLOSED bytes=$($buf.Length) log=$LogPath"
}
