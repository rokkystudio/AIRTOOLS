...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%ROOT%\services\connectivity\build-daemon.ps1"
if errorlevel 1 exit /b %errorlevel%

echo.
echo [2/3] Building modified mtd4...
"%PY%" "%ROOT%\services\connectivity\build-mtd4.py"
if errorlevel 1 exit /b %errorlevel%

echo.
echo [3/3] Running mandatory preflight...
"%PY%" "%ROOT%\services\connectivity\verify-mtd4.py"
if errorlevel 1 exit /b %errorlevel%
...
echo.
echo DO NOT flash it persistently before uart-test-boot.py succeeds.
exit /b 0
