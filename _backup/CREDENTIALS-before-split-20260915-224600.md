...
- Через UART получен shell без ввода пароля.
- Из локального `/etc/passwd` получен hash пользователя `molink`.
- Пароль был восстановлен hashcat.
...
Данные входа для нашего экземпляра:
...
Этот раздел содержит только то, что было получено на нашем экземпляре через фото, UART, Windows/USB/Wi-Fi проверки и локальный перебор hash.
...
UART даёт shell без штатного Telnet-логина.
...
На нашем устройстве `/etc/shadow` и `/etc_ro/shadow` не найдены. Hash пароля находится в `/etc/passwd`.

Локальная строка пользователя в README не дублируется целиком, чтобы не размножать hash. Оригинал находится в UART-логах и в файлах hashcat.

Расшифровка учётной записи:
...
Hash — classic Unix DES crypt. Это не plaintext-пароль.

## Как восстанавливали пароль
...
Сначала был перебран диапазон длины `1..7`:
...
Затем пароль был найден на длине `8`:
...
Результат хранится локально:

```text
hashcat\hashcat-found.txt
```

В README пароль не дублируется. Для просмотра результата использовать:
